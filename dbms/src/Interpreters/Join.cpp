...
namespace
{
    ...
    struct Adder<fill_left, ASTTableJoin::Strictness::Asof, Map>
    {
        static void addFound(const typename Map::mapped_type & mapped, size_t num_columns_to_add, MutableColumns & added_columns,
                             size_t i, IColumn::Filter & filter, IColumn::Offset & /*current_offset*/, IColumn::Offsets * /*offsets*/,
                             const std::vector<size_t> & right_indexes, const Join::ASOFTimeType asof_key)
        {
            if ( auto v = mapped.find_asof(asof_key) ) {
                ...
                for (size_t j = 0; j < num_columns_to_add; ++j)
                    added_columns[j]->insertFrom(*res.second.block->getByPosition(right_indexes[j]).column, res.second.row_num);
            }
            else
            {
        ...
        {
            filter[i] = 0;
            std::cout << "Adder::addNotFound " << " i=" << i << std::endl;
                ...
                if (find_result.isFound())
                {
                    ...
                    if constexpr (STRICTNESS == ASTTableJoin::Strictness::Asof) {
                        ...
                        Adder<Join::KindTrait<KIND>::fill_left, STRICTNESS, Map>::addFound(
                            mapped, num_columns_to_add, added_columns, i, filter, current_offset, offsets_to_replicate.get(), right_indexes, asof_key);
                    } else {
                        mapped.setUsed();
                        Adder<Join::KindTrait<KIND>::fill_left, STRICTNESS, Map>::addFound(
    ...
    const Maps & maps_) const
{
    ...
    if (strictness == ASTTableJoin::Strictness::Any || strictness == ASTTableJoin::Strictness::Asof)
    {
